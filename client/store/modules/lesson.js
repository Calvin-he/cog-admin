...
const actions = {
  listLessons ({ commit }) {
    return Vue.axios
      .get('/lessons')
      .then(response => commit('LISTOFLESSONS', response.data))
  },

  addLesson ({ commit }, lesson) {
    return Vue.axios
      .post('/lessons', lesson)
      .then(response => commit('ADDLESSON', response.data))
  },

  updateLesson ({ commit }, lesson) {
    return Vue.axios
      .put('/lessons/' + lesson._id, lesson)
      .then(response => commit('UPDATELESSON', response.data))
  },
  ...
  listLessonsOfSeries ({ commit }, { seriesId }) {},

  listSeries ({ commit }) {
    return Vue.axios
      .get('/series')
      .then(response => commit('LISTOFSERIES', response.data))
  },
  ...
  listMedia ({ commit, state }) {
    if (state.mediaList.length === 0) {
      return Vue.axios
        .get('/media')
        .then(response => commit('LISTOFMEDIA', response.data))
    } else {
  ...
  addMedia ({ commit }, file) {
    ...
    return Vue.axios.post('/media/upload', formData).then(response => {
